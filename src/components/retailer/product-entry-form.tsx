...
export function ProductEntryForm() {
    const [products, setProducts] = useState<ProductItem[]>([
        { id: 1, alias: '', product: '', quantity: 1, price: 0 },
    ]);
    ...
    const updateProduct = (id: number, field: keyof ProductItem, value: any) => {
        const updatedProducts = products.map((p) => {
            if (p.id === id) {
                // If updating the alias, check if we have a history match
                if (field === 'alias') {
                    const historyMatch = retailerProductHistory.find(
                        (h) => h.alias.toLowerCase() === value.toLowerCase()
                    );
                    if (historyMatch) {
                        ...
                        <div key={product.id} className="grid grid-cols-12 gap-2 border-b p-3">
                            <div className="col-span-3">
                                <Input
                                    ...
                                    value={product.alias}
                                    onChange={(e) =>
                                        updateProduct(product.id, 'alias', e.target.value)
                                    }
                                />
                                {retailerProductHistory.some(
                                    (h) => h.alias.toLowerCase() === product.alias.toLowerCase()
                                ) && (
